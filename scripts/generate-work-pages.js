/* ============================================================
   ...
   ------------------------------------------------------------
   Generate satu halaman statis per karya di w/<slug>/index.html
   (folder /w/ di root, TERPISAH dari works/), supaya:
     1. Tiap karya punya URL sendiri yang bisa di-share
        (https://rinaldisign.com/w/dn1/, dst).
     ...
     4. Folder works/ (index.html, css/, js/, works-data.js) TIDAK
        pernah kecampuran ribuan folder hasil generate — jadi tetap
        rapi & gampang di-develop walau karyanya sampai 1000.
...
const OUTPUT_DIR = path.join(REPO_ROOT, 'w'); // folder terpisah, bukan di dalam works/
...
function run() {
  ...
  console.log(`✔ ${count} halaman karya berhasil digenerate di w/<slug>/index.html`);
}
