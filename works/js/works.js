...
/* Kalau datang dari widget Featured di homepage (?q=DN.12), otomatis
   isi & jalankan pencarian supaya karya itu langsung kelihatan. */
...
/* ---------- Slug per karya (dipakai untuk URL yang bisa di-share) ----------
   Contoh: catno "DN.1" -> slug "dn1". Diturunkan otomatis dari catno,
   supaya works-data.js tidak perlu field tambahan apa pun. */
function slugify(catno){
  return (catno || '').replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
}

function findIndexBySlug(slug){
  return WORKS.findIndex(w => slugify(w.catno) === slug);
}
...
const mSpotify = document.getElementById('mSpotify');
const mAppleMusic = document.getElementById('mAppleMusic');
const mYoutubeMusic = document.getElementById('mYoutubeMusic');
...
/* Tampilkan/sembunyikan satu link platform (Spotify/Apple Music/dst)
   tergantung field itu diisi atau tidak di works-data.js. Kosongkan
   field-nya (atau hapus) -> ikon & link otomatis tidak muncul. */
function setPlatformLink(el, url){
  if (url && url !== '#') {
    el.style.display = '';
    el.href = url;
  } else {
...
function openWork(index, opts = {}){
  const w = WORKS[index];
  if (!w) return;

  const slug = slugify(w.catno);
  const targetPath = slug ? `/works/${slug}/` : null;
  if (targetPath) {
    if (opts.replace) {
      history.replaceState({ work: slug }, '', targetPath);
    } else if (window.location.pathname !== targetPath) {
      history.pushState({ work: slug }, '', targetPath);
    }
  }
  ...
  setPlatformLink(mSpotify, w.spotify);
  setPlatformLink(mAppleMusic, w.appleMusic);
  setPlatformLink(mYoutubeMusic, w.youtubeMusic);
...
function closeWork(opts = {}){
  ...
  const onWorkUrl = /^\/works\/[^/]+\/?$/.test(window.location.pathname) &&
                     window.location.pathname !== '/works/';
  if (!opts.skipHistory && onWorkUrl) {
    history.pushState({ work: null }, '', '/works/');
  }
}

/* Tombol back/forward browser -> buka/tutup popup mengikuti riwayat URL */
window.addEventListener('popstate', (e) => {
  const slug = e.state && e.state.work;
  if (slug) {
    const idx = findIndexBySlug(slug);
    if (idx >= 0) { openWork(idx, { replace: true }); return; }
  }
  closeWork({ skipHistory: true });
});

/* Kalau halaman ini adalah halaman statis per-karya hasil generator
   (lihat scripts/generate-work-pages.js), window.__OPEN_WORK__ berisi
   slug karyanya -> otomatis buka popup itu saat halaman dimuat. */
if (typeof window.__OPEN_WORK__ === 'string') {
  const initialIndex = findIndexBySlug(window.__OPEN_WORK__);
  if (initialIndex >= 0) openWork(initialIndex, { replace: true });
}

/* ---------- Copy link (tombol di dalam popup) ---------- */
...
if (mCopyLink) {
  mCopyLink.addEventListener('click', async () => {
    ...
    try {
      ...
      window.prompt('Copy link:', url); // fallback kalau clipboard API diblokir
      return;
    }
    mCopyLink.classList.add('copied');
    mCopyLink.title = 'Link copied!';
    setTimeout(() => {
      mCopyLink.classList.remove('copied');
      mCopyLink.title = 'Copy link to this work';
    }, 1600);
  });
}
