...
function renderRows(){
  ...
  if (filteredWorks.length === 0) {
    compList.innerHTML = `<p class="no-results">Tidak ada karya yang cocok. Coba nomor katalog (misal “DN.12”) atau sepotong judulnya.</p>`;
    return;
  }
...
function updateResultCount(){
  ...
  resultCount.textContent = q ? `${filteredWorks.length} hasil untuk "${q}"` : '';
}
